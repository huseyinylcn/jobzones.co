...
const fs = require("fs");
 ...
 let { update, updateProfileIMG, updateBannerIMG } = require('../../model/user/update')
 let {img, imgBanner} = require('../../model/user/img')
...
try {
    if(req.user){
        ...
        userGET(req.user).then(data=>{
            ...
            res.json({result:0,message:`Kullanıcı biligleri veri tabanından gelemedi System Error ${err}`})
        })
    }else{
        res.json({result:0,message:`Kullsanıcı giriş yapmamış giriş yaptır`})
    }
} catch (error) {
...
router.post("/img",(req,res,next)=>{
    ...
    try {
        img(req,res,(err)=>{
            if(err) {
                updateProfileIMG(req.user,`/img/default.jpg`).then(data=>{
                    res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine geldi tekrar dene ${err}`})
                }).catch((errors=>{
                    res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine gelemedi ${errors}`})
                }))
            }
            else next()
        })
    } catch (error) {
        res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine gelemedi system error!! ${error}`})
    }
...
router.post("/img",(req,res,next)=>{
    ...
    try {
        console.log(3)
        updateProfileIMG(req.user, `/ProfilPicture/profileImg/${path.basename(req.file.filename)}`).then(data=>{
          
            console.log(path.basename(req.file.filename))
            res.json({result:1,messsage:"successfull"})
        }).catch((err)=>{
            res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine gelemedi end  ${err}`})
        })
...
router.post("/banner",(req,res,next)=>{
    ...
    try {
        imgBanner(req,res,(err)=>{
            if(err) {
                updateBannerIMG(req.user,`/img/default.jpg`).then(data=>{
                    res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine geldi tekrar dene ${err}`})
                }).catch((errors=>{
                    res.json({result:0,message:`girilen fotoğraf kayıt edilemedi ve eski fotoğraf silindi default yerine gelemedi ${errors}`})
                }))
...
router.post("/banner",(req,res,next)=>{
    ...
    try {
        console.log(3)
        updateBannerIMG(req.user, `/ProfilPicture/banner/${path.basename(req.file.filename)}`).then(data=>{
...
router.post("/", (req, res, next) => {
    try {
        ...
        info.dayandtimework = (JSON.stringify(req.body.dayandtimework)).replace(/'/g, '"')
...
router.post('/',(req,res,next)=>{
    try {
        update(req.user,info).then((data)=>{
            console.log(data)
